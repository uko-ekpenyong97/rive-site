...
- **Closed tiles gain autoplaying video loops** (2026-07-30). The bento cells stop showing a `MEDIA` placeholder and start showing the product. **No modal file was touched** — §1's rule that a hero holds a live `.riv` and never a video is about the sheet, and it stands. A tile is a different slot: these are recordings of real products running Rive, and the live artifact is still one click behind them.
  ...
  - **Campaigns is held deliberately.** Its clip is Spotify Wrapped, which would reverse the 2026-07-26 Strava re-anchor above — the entry that exists precisely to keep Spotify material inside CaseStudies. Geometry independently rules it out: a 4:3 source into that cell's ~3.7:1 media slot loses ~64% of its height. The `.mp4` stays committed so the call is reversible without a re-fetch; the cell keeps its labelled placeholder, and `tileMedia` is optional so that reads as a designed state rather than a gap.
    - ~~**FOLLOW-UP, not addressed here:**~~ **RESOLVED 2026-07-31** (see the entry below). The campaigns cell's copy read "Wrapped moments, made personal" / "Interactive year-in-review campaigns, personalized for millions." The title is now "Recaps that know who you are"; the description was already brand-neutral and is unchanged.
...
- **Campaigns tile resolved — Strava footage, and the re-anchor confirmed rather than reversed** (2026-07-31). The held tile from the entry above is wired; **all six cells now carry media** and no `MEDIA` placeholder survives in the section. No modal file was touched — this modal stays heroless, and the tile is the cell.
  - **The Strava re-anchor is CONFIRMED, not overturned.** This session first considered a Spotify + LinkedIn back-to-back sequence for the cell and **rejected it** once the Strava asset surfaced: the whole point of the 2026-07-26 entry is that Spotify material stays inside CaseStudies, and a tile leading with it would have quietly undone that. The new clip resolves the tension in the other direction — it makes the cell *agree* with the modal behind it instead of competing with it.
  - **Asset replaced.** `public/video/use-cases/campaigns.mp4` was Spotify Wrapped, from `https://framerusercontent.com/assets/8aJN4DoBcfmZ8aVXsC2lqfLDik.mp4`; it is now Strava Year in Sport, from `https://framerusercontent.com/assets/QiGJHCv8kOmhp5xw2Zp5wKSNXzY.mp4`. The old URL is recorded here **so the bytes stay recoverable without the repo carrying dead weight** — git history has the file, and the escrow rationale expired the moment the tile was resolved. Net −259,533 B (770,604 → 511,071). No audio track, so nothing here for the polish-phase ffmpeg pass to strip.
  - **PROVENANCE QUIRK — do not "fix" this asset.** `rive.app/use-cases/product-design` serves the clip under a card titled **"LinkedIn Year in Review"**. That is Rive's own mislabel. The footage is verified Strava frame-by-frame: Strava orange, the Strava wordmark, the "YEAR IN SPORT" lockup. The origin card's label is wrong; the asset is right. Recorded in the source map at the top of `useCaseContent.ts` as well, because that is where someone would go to "correct" it.
  - **The IP sentence in the 2026-07-26 entry is amended, not deleted** (struck in place above). "No Strava artwork is used (their IP)" was true when written and is not now. The new basis, stated plainly: the clip is published by Rive on their own use-cases page as customer showcase material — the same sourcing basis as the other five tiles and as the CaseStudies posters.
  - **Geometry: the steepest crop of the six, and it survives.** 1066×600 into a **measured** 922×246 slot (3.748:1 — measured off the live DOM, not quoted) discards **52.6% of source height**. It works because the phone is centred and the type is large. Six anchors were rendered at true tile size and looked at: `50%` keeps the punchline but clips the title card's route glyph; `35%` and `42%` fix the title card but cut **"like you."** mid-sentence, which reads as broken rather than cropped. **`50% 46%`** is the only anchor holding all three beats of the clip, and it is pinned in the test set with the other five. No `crop` — this is a clean render, with nothing burnt into the frame.
  - **Poster at 2.5s**, the narrowest window of the six: t=0 is black, ~2.05s catches the title card mid-wipe, ~3.5s is an empty teal transition, ~4.06s has the stats mid-animation. 2.5s is the settled *"But nobody did it quite like you."* frame — brand-orange, phone legible, and it states the thesis in words, which is what a permanent reduced-motion still should do.
  - **Cell retitled** to **"Recaps that know who you are"**, resolving the follow-up left open above. "Wrapped" is Spotify's word and was visibly wrong over Strava footage; the personalization-at-scale angle it was protecting is kept. The description was already brand-neutral and is unchanged, so this is one edit, not three. Pins updated in `modalSection.test.tsx` and the Figma handoff doc's "copy as shipped".
  - **Test-design note, deliberate:** with every cell wired, the section-level placeholder assertion inverts to **0**, so the absent-asset fallback is now proven against a standalone `<BentoCell>` rather than against a real media-less cell. `tileMedia` stays **optional** and a guard asserts so — a lite modal designed without a tile is still a state this model can express, which is exactly what Campaigns was for one session.
  - **⚠ HEADLESS CHROME DOES NOT SCREENSHOT `<video>`.** `Page.captureScreenshot` over a tile returns **pure black** while the element reports `readyState=4`, `paused=false` and a live `currentTime` — reproduced under two different flag sets (with and without `--use-gl=angle --use-angle=swiftshader`). It is a compositing limitation, **not** a broken tile, and it will look exactly like a broken tile to whoever hits it next. Tell the two apart the way `verify-pixels.mjs` does: `drawImage` the live element into a canvas and read the buffer back — that samples the decoded frame directly and is unaffected by the compositor. Measured that way, all six tiles paint and advance (campaigns luma 42.5 → 68.8 across a one-second gap). A tile's *anchor* is therefore verified by three things together: the computed style (`object-position: 50% 46%` confirmed on the live node), the pixel check above, and an exact-math crop render at true tile size looked at directly — not by a screenshot.
