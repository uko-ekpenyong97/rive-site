...
- IMPORTANT: **`sips` cannot encode AVIF for the web.** It writes a file that reports correct dimensions and passes `img.decode()`, but paints fully transparent in Chrome (`meanLuma 0, meanAlpha 0`) — an invisible poster that no dimension check, decode check or build step would catch. Encode with `sharp` instead (`npm i --no-save sharp`, so it never enters the manifest for a one-off conversion). Always verify a converted image by drawing it to a canvas and reading back mean luminance; decoding successfully is not the same as having pixels.
- Case-study posters: the slot is ~3.3:1 against 16:9 sources, so `cover` discards about half the height. Use `npm run analyze:crop <image> [--at N]` to get the row arithmetic, then **look at rendered candidates before choosing** — on both posters anchored so far, the metric's optimum produced a worse picture than the chosen anchor. Anchors are pinned as an exact set in `src/__tests__/caseStudies.test.tsx`.
