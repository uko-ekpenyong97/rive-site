...
/* ──────────────────────────────────────────────────────────────────────────
 * TILE MEDIA — SOURCE MAP
 *
 * The CLOSED bento tile only. The modal hero slot is untouched by this: spec §1
 * is unambiguous that a hero holds a live .riv and never a video, because video
 * of interactivity is the After Effects worldview. A tile is a different slot —
 * these are recordings of real products running Rive, shown at a glance, and the
 * live artifact is still one click behind them.
 *
 * Files are committed under public/video/use-cases/ and referenced by LITERAL
 * PATH, never imported. An import would route the bytes through Vite's asset
 * pipeline and put a hashed URL in the JS chunk; a public/ path is copied
 * verbatim and fetched by the browser, so the video weight can never reach the
 * bundle. Posters are generated by `npm run posters:video`.
 *
 * WHAT EACH CLIP ACTUALLY IS — verified by decoding four frames per file in
 * Chrome and looking at them, because three of the six are not what their
 * filenames suggest:
 *
 *   product-ui.mp4  ← https://cdn.rive.app/framer/product_ui2.mp4
 *     A smart-home wall display waking up: dark screen → New York, 59°, 02:30 PM,
 *     next meeting. Product UI on real hardware, not a mockup.
 *     NOTE: this is the one clip carrying an AUDIO track (parsed from its `hdlr`
 *     boxes; the other five are video-only). It is rendered `muted` and has no
 *     controls, so it never plays — but if the film-tv re-crop below is ever
 *     done with ffmpeg, strip this track in the same pass.
 *     Poster grabbed at 1.9s, NOT 0s: the clip opens on the screen switched OFF,
 *     so a t=0 poster is a black tile for exactly the reduced-motion visitors
 *     the poster exists for.
 *
 *   game-ui.mp4  ← https://cdn.rive.app/framer/joel_home.mp4
 *     A 2D game in play: segmented health bar pinned top-centre, a horde closing
 *     in, floating combat text ("PEW", "PING", "SPROOO!! x1") over the fight.
 *     HUD and damage numbers driven live. The title is not identifiable from the
 *     footage and is deliberately not guessed at here.
 *
 *   websites.mp4  ← https://cdn.rive.app/framer/figma_rive.mp4
 *     Figma's own homepage — "How you design, align, and build matters" — with
 *     the multiplayer canvas demo below it, cursors and labels moving.
 *     KNOWN: mean luma 211, i.e. a white page in a dark-only grid. Shipped as
 *     is; taming it is a browser-pass call, and the knob would live next to
 *     `crop` below rather than as an overlay element.
 *
 *   automotive.mp4  ← https://cdn.rive.app/framer/data_driven2.mp4
 *     An instrument cluster booting behind a steering wheel: R N D gear
 *     selector, 00 MPH, ECO, 446 Kw, 323 M range, media widget. Data-bound
 *     vehicle UI. Poster grabbed at 2.6s, once the cluster is fully populated.
 *
 *   film-tv.mp4  ← https://cdn.rive.app/framer/broadcast.mp4
 *     A live mahjong esports broadcast (Smahjong) with a four-player scoreboard
 *     overlay updating against the table.
 *     KNOWN: the capture is a full BROWSER WINDOW, so the source frame carries
 *     YouTube chrome — title bar, scrubber, timecode, "Screenshot" button. The
 *     tile slot crops width, not height, so the chrome would survive untouched.
 *     `crop` below trims it in CSS. Measured on the decoded frame: the chrome
 *     occupies the top ~7.2% and bottom ~6.0% of the source height; the values
 *     used add a small safety margin.
 *     POLISH-PHASE ALTERNATIVE, if the CSS zoom reads soft in a browser pass:
 *     re-encode a real crop with ffmpeg (not installed here, and macOS ships
 *     none — see scripts/lib/verify-pixels.mjs) using those same measurements —
...
/**
 * Trims capture chrome baked into a source clip, as fractions of the media box
 * height. Applied in CSS (zoom + shift) rather than by re-encoding, so the
 * committed bytes stay the ones that were downloaded and the correction stays
 * reviewable as data.
 */
export interface TileMediaCrop {
  top: number;
  bottom: number;
}

/**
 * The autoplaying loop behind a CLOSED bento tile. Optional per use case: a cell
 * with no `tileMedia` keeps BentoCell's labelled MEDIA placeholder, which is a
 * real state here and not a gap (see campaigns in the source map above).
 */
export interface TileMedia {
  type: "video";
  /** public/ path, NOT an import — see the source map. */
  src: string;
  /** Generated by `npm run posters:video`; the reduced-motion still. */
  poster: string;
  /**
   * `object-position` for the cover crop. Omitted means centred. Every tile slot
   * has a different aspect ratio from its source, so this is the one dial that
   * decides which part of the footage survives.
   */
  objectPosition?: string;
  /** Only where the source has chrome burnt into it. */
  crop?: TileMediaCrop;
}
...
export interface UseCaseContent {
  ...
  hero?: UseCaseHero;
  /**
   * The closed tile's autoplaying loop (see the TILE MEDIA source map). Optional
   * and independent of `hero`: this is the bento cell, not the sheet.
   */
  tileMedia?: TileMedia;
  proof: ProofItem[];
  quote?: UseCaseQuote;
...
export const USE_CASES: UseCaseContent[] = [
  /* ---------------------------------------------------------- Tier 1 ----- */
  {
    ...
    /* Slot is ~2.7:1 against a 16:9 source, so cover discards ~34% of the
       height. The display is centred, so centred is right. */
    tileMedia: {
      type: "video",
      src: "/video/use-cases/product-ui.mp4",
      poster: "/video/use-cases/posters/product-ui.avif",
    },
    proof: [
      {
        source: "Notion",
  ...
  },
  {
    ...
    /* ~1.1:1 slot against a 4:3 source — cover trims ~16% of the width. The
       player and the health bar both sit centre, so nothing load-bearing goes. */
    tileMedia: {
      type: "video",
      src: "/video/use-cases/game-ui.mp4",
      poster: "/video/use-cases/posters/game-ui.avif",
    },
    proof: [
      {
  ...
  /* ---------------------------------------------------------- Tier 2 ----- */
  {
    ...
    /* Cover trims ~14% of the height here. Biased DOWN (60%) rather than centred
       so the crop comes off Figma's nav rather than off the canvas demo — the
       moving cursors are the part that is actually running Rive. */
    tileMedia: {
      type: "video",
      src: "/video/use-cases/websites.mp4",
      poster: "/video/use-cases/posters/websites.avif",
  ...
  },
  {
    ...
    /* The widest mismatch of the set: a 16:9 source into a ~1.4:1 slot, so cover
       takes ~23% off the width. The cluster spans nearly the full frame, so the
       outer readouts (446 Kw, 323 M) clip no matter where this is anchored —
       centred keeps the gear selector and the vehicle, which carry the idea. */
    tileMedia: {
      type: "video",
      src: "/video/use-cases/automotive.mp4",
      poster: "/video/use-cases/posters/automotive.avif",
    },
    proof: [
      {
        source: "Embedded",
  ...
  },
  {
    ...
    /* The only clip needing a crop: the source is a browser-window capture, so
       YouTube chrome is burnt into the top and bottom of the frame. Cover trims
       WIDTH here (~11%), which would leave that chrome fully intact — hence the
       explicit trim. Measured 7.2% top / 6.0% bottom on the decoded frame;
       these carry a small safety margin. */
    tileMedia: {
      type: "video",
      src: "/video/use-cases/film-tv.mp4",
      poster: "/video/use-cases/posters/film-tv.avif",
      crop: { top: 0.075, bottom: 0.065 },
    },
    proof: [
      {
        source: "Live data",
